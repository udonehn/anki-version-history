...
Changes saved here apply immediately — no restart needed.

- **auto_capture** (default `true`): automatically capture a version after
  every undoable change (edits, find & replace, undo/redo, …) and after sync.
  Manual snapshots keep working when this is off.
...
- **heartbeat_scan_minutes** (default `5`): periodic background scan that
  catches changes arriving without a normal operation (e.g. made by other
  add-ons). `0` disables the heartbeat.
- **capture_media** (default `true`): media capture is **disabled in this
  release** — this setting has no effect until a future update enables it.
- **media_scan_on_profile_open** (default `true`): no effect while media
  capture is disabled.
